...
fun configurePluginSubproject(p: Project) {
    p.tasks.matching { it.name.contains("AarMetadata") }.configureEach {
        enabled = false
    }
...
subprojects {
    // Tránh gọi afterEvaluate trên project đã được evaluated (như :app)
    if (state.executed) {
        configurePluginSubproject(this)
    } else {
        afterEvaluate {
            configurePluginSubproject(this)
        }
